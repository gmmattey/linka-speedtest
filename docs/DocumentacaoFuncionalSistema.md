...
│   ├── DiagnosticScreen    ← diagnóstico de 6 áreas (via "Diagnóstico")
│   ├── RecommendScreen     ← recomendações (via "Recomendações")
│   ├── DetailsScreen       ← métricas técnicas detalhadas: qualidade sob carga, provedor (via "Detalhes")
│   └── ExploreScreen       ← hub de ferramentas avançadas (via "Ferramentas")
│       ├── DiagnosticScreen
│       ├── RecommendScreen
...
ResultScreen → [Diagnóstico]    → DiagnosticScreen    → [‹ Voltar] → ResultScreen
ResultScreen → [Recomendações]  → RecommendScreen     → [‹ Voltar] → ResultScreen
ResultScreen → [Detalhes]       → DetailsScreen       → [‹ Voltar] → ResultScreen
ResultScreen → [Ferramentas]    → ExploreScreen       → [‹ Voltar] → ResultScreen
...
Botão circular `200px × 200px`, `border-radius: 50%`, outlined (`border: 1.5px solid var(--accent)`), fundo transparente. Texto "Iniciar" em Geist 600 20px na cor accent. Dois pseudo-elementos pulsantes (`::before`, `::after`) animam `lkOrbPulse` com offset de 1,2 s entre eles.
...
│                                  │
│  [🎮 Jogos  Bom] [📺 4K  Bom]   │
│  [💼 Office Bom] [📹 Vídeo Bom] │
│                                  │
│  DIAGNÓSTICO DA CONEXÃO          │  ← kicker uppercase
│  Wi-Fi limitando a conexão       │  ← título (linguagem leiga)
│  O que fazer agora:              │
│  Reiniciar o roteador            │  ← ação imediata em destaque
│                                  │
│  Explorar                        │
│  🟣 Diagnóstico              ›   │
│  🟣 Recomendações            ›   │
...
Quatro células em grid (`lk-result__metric-grid`), cada uma com:
- Rótulo (`Download`, `Upload`, `Resposta`, `Oscilação`)
- Valor numérico grande (DL/UL em Mbps; latência/jitter em ms)
- Badge de nota `A–F · Label` com cor de fundo por tier:

| Nota | Intervalo DL | Intervalo UL | Intervalo Latência | Intervalo Jitter | Background |
|---|---|---|---|---|---|
| A | ≥ 100 Mbps | ≥ 30 Mbps | ≤ 30 ms | ≤ 5 ms | `--color-good-bg` |
| B | ≥ 25 Mbps  | ≥ 10 Mbps | ≤ 60 ms | ≤ 15 ms | `--color-good-bg` |
| C | ≥ 10 Mbps  | ≥ 5 Mbps  | ≤ 100 ms | ≤ 30 ms | `--color-warn-bg` |
| D | ≥ 5 Mbps   | ≥ 1 Mbps  | ≤ 200 ms | ≤ 50 ms | `--color-bad-bg` |
| F | < 5 Mbps   | < 1 Mbps  | > 200 ms | > 50 ms | `--color-bad-bg` |

Labels: A=Excelente, B=Bom, C=Regular, D=Ruim, F=Crítico.  
Cor do valor numérico: DL usa `--dl` (azul), UL usa `--ul` (verde), latência e jitter usam `--text`.

### Linha de cenários (`lk-result__use-row`)

4 ícones SVG em flex horizontal, sem título. Cada item: ícone circular com cor por status + label do caso de uso + chip de status. Visível apenas quando `interpreted.useCases.length > 0`.

Casos de uso: Jogos (`game`), 4K (`stream`), Home Office (`work`), Vídeo (`videoCall`).

Labels de status:
| Status interno | Label chip |
|---|---|
| `good` | "Bom" |
| `maybe` | "Atenção" |
| `bad` | "Ruim" |

Thresholds de avaliação: definidos em `src/core/profiles.ts` via `interpretSpeedTestResult()`.

### Bloco Combined Diagnosis (`lk-result__combined`)

Exibido logo após a linha de cenários. Sempre visível. Gerado por `combineDiagnostics()` em `src/utils/combinedDiagnosis.ts`.

Campos exibidos (simplificado — sem "Explicação" e sem "Confiança"):
- **Kicker** — "Diagnóstico da conexão" (uppercase, `--text-3`)
- **Título** — causa em linguagem leiga (ex.: "Wi-Fi limitando a conexão")
- **O que fazer agora** — ação imediata recomendada em destaque bold
...
1. Gera card visual 1080×540 px via `generateShareCard()` (canvas com DL/UL/latência, fundo dark, barra accent).
...
A seção Explorar na ResultScreen exibe até 4 itens em IOSList, condicionais por prop:

| Item | Ícone | Cor ícone | Background | Prop |
|---|---|---|---|---|
| Diagnóstico | `shield` | `#fff` | `var(--accent)` (roxo) | `onDiagnostic` |
| Recomendações | `bulb` | `#fff` | `var(--accent)` (roxo) | `onRecommend` |
| Detalhes | `document` | `var(--text-2)` | `var(--surface-3)` (cinza) | `onDetails` |
| Ferramentas | `cog` | `var(--text-2)` | `var(--surface-3)` (cinza) | `onExplore` |

Sem subtítulos nos itens. Hierarquia de cor: análise da conexão (roxo) vs. utilitários (cinza).

### Serviços consumidos

- Props: `result`, `server`, `unit`, `theme`, `onToggleTheme`, `onRetry`, `onShowHistory`, `onDiagnostic?`, `onRecommend?`, `onDetails?`, `onExplore?`, `connectionType?`, `contractedDown?`, `contractedUp?`, `onUpdateContracted?`
